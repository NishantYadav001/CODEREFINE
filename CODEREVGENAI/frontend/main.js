...
const initApp = () => {
    ...
    try {
        ...
        // Admin Role Check - Redirect Profile to Admin Dashboard
        const role = localStorage.getItem('role') || sessionStorage.getItem('role');
        if (role === 'admin') {
            const profileLink = get('profile-link');
            if (profileLink) {
                profileLink.href = '/admin';
                profileLink.title = 'Admin Dashboard';
                profileLink.innerHTML = '<i class="fas fa-shield-alt"></i>';
            }
            
            const adminBtn = get('admin-dashboard-btn');
            if (adminBtn) adminBtn.classList.remove('hidden');

            // Update Badge for Admin
            const userBadge = get('user-badge');
            if (userBadge) {
                userBadge.innerText = 'Admin';
                userBadge.className = 'px-3 py-1 rounded-full bg-red-900/50 text-red-200 text-xs font-bold border border-red-700';
            }
        }
        ...
        async function processCode(action) {
            ...
            try {
                ...
                if (feedback) {
                    const rawHtml = typeof marked !== 'undefined' ? marked.parse(data.review) : data.review;
                    // Sanitize HTML before rendering
                    feedback.innerHTML = typeof DOMPurify !== 'undefined' ? DOMPurify.sanitize(rawHtml) : rawHtml;
                }
                
                // ALWAYS update downloadData so other buttons work
                window.downloadData = {
                    code: action === 'fix' ? data.rewritten_code : code,
                    review: data.review,
                    language: language,
                    originalCode: code,
                    stats: data.stats,
                ...
                if (msg.includes("401")) msg = "Session expired. Please login again.";
        ...
        async function formatCode() {
            ...
            try {
                ...
                if (parser && window.prettier && window.prettierPlugins) {
                    ...
                    // Use AI for other languages (Python, Java, etc.)
                    showToast("Formatting with AI...", "info");
                    ...
                    if(formatted) {
                        window.monacoEditor.setValue(formatted);
                        showToast("Code formatted with AI!", "success");
                    } else {
                    ...
                    chatHistory.innerHTML += `
                        <div class="flex gap-3">
                            ...
                            <div class="bg-slate-700/50 p-3 rounded-2xl rounded-tl-none text-sm text-slate-300 border border-slate-600 prose prose-invert prose-sm max-w-none">
                                ${typeof DOMPurify !== 'undefined' ? DOMPurify.sanitize(typeof marked !== 'undefined' ? marked.parse(aiMsg) : aiMsg) : aiMsg}
                            </div>
                        </div>`;
                    chatHistory.scrollTop = chatHistory.scrollHeight;
                } catch(e) {
        ...
        // Buttons
        setClick('modeGenerate', () => window.location.href = '/generate');
        ...
        setClick('generateTestsBtn', async () => {
            ...
            const feedback = get('reviewFeedback');
            if (feedback) feedback.innerHTML = '<p class="text-green-400"><i class="fas fa-spinner animate-spin mr-2"></i>Generating tests...</p>';
            try {
                ...
                const data = await res.json();
                const safeTests = typeof DOMPurify !== 'undefined' ? DOMPurify.sanitize(typeof marked !== 'undefined' ? marked.parse(data.test_cases) : data.test_cases) : data.test_cases;
                if (feedback) feedback.innerHTML = `<h4 class="text-green-400 font-bold mb-2">Generated Tests</h4><div class="bg-slate-800 p-3 rounded max-h-96 overflow-auto">${safeTests}</div>`;
        ...
        setClick('generateDocsBtn', async () => {
            ...
            try {
                ...
                const data = await res.json();
                const safeDocs = typeof DOMPurify !== 'undefined' ? DOMPurify.sanitize(typeof marked !== 'undefined' ? marked.parse(data.documentation) : data.documentation) : data.documentation;
                if (feedback) feedback.innerHTML = `<h4 class="text-blue-400 font-bold mb-2">Documentation</h4><div class="bg-slate-800 p-3 rounded max-h-96 overflow-auto">${safeDocs}</div>`;
        ...
        setClick('securityScanBtn', async () => {
            ...
            try {
                ...
                const data = await res.json();
                const safeAnalysis = typeof DOMPurify !== 'undefined' ? DOMPurify.sanitize(data.security_analysis) : data.security_analysis.replace(/</g, '&lt;');
                if (feedback) feedback.innerHTML = `<h4 class="text-red-400 font-bold mb-2">Security Analysis</h4><div class="bg-slate-800 p-3 rounded max-h-96 overflow-auto whitespace-pre-wrap text-sm">${safeAnalysis}</div>`;
        ...
        setClick('refactorSuggestionsBtn', async () => {
            ...
            try {
                ...
                const data = await res.json();
                const safeSuggestions = typeof DOMPurify !== 'undefined' ? DOMPurify.sanitize(typeof marked !== 'undefined' ? marked.parse(data.refactoring_suggestions) : data.refactoring_suggestions) : data.refactoring_suggestions;
                if (feedback) feedback.innerHTML = `<h4 class="text-purple-400 font-bold mb-2">Refactoring Suggestions</h4><div class="bg-slate-800 p-3 rounded max-h-96 overflow-auto prose prose-invert prose-sm max-w-none">${safeSuggestions}</div>`;
            } catch (e) { showToast("Analysis failed", "error"); }
        });

        setClick('saveSnippetBtn', () => {
            const code = window.downloadData ? window.downloadData.code : (window.monacoEditor ? window.monacoEditor.getValue() : "");
            const language = window.downloadData ? window.downloadData.language : (window.monacoEditor ? window.monacoEditor.getModel().getLanguageId() : "python");
            
            if (!code.trim()) return showToast('Please enter code first', 'warning');
            ...
            fetch('/api/snippets/save', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ user: userType, title, code, language })
            }).then(res => res.json()).then(data => {
                showToast(`✅ Snippet saved as #${data.snippet_id}`, 'success');
            }).catch(err => showToast("Save failed", "error"));
        });

        setClick('historyBtn', async () => {
            const feedback = get('reviewFeedback');
            try {
